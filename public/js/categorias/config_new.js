...
const State = {
    // Datos de la Categoría y Contexto
    categoriaId: window.serverCategoriaId,
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log("🚀 Iniciando Configurador de Categoría v2");
...
async function fetchConfig() {
    try {
        // Step 1: List all category templates
        const listUrl = `${State.basePath}api/categorias/plantillas.php?action=list&categoria_id=${State.categoriaId}`;
        const listRes = await fetch(listUrl);
        const listData = await listRes.json();

        if (!listData.success) {
            console.warn('API list response:', listData.message || 'sin datos');
            // Still proceed to render the empty state — don't block the UI
            State.templates = [];
        } else {
            State.templates = listData.plantillas || [];

            // Update group info for mock previews
            if (listData.grupo_info) {
                State.mockData.grupo = listData.grupo_info.grupo_nombre || State.mockData.grupo;
            }
        }

        // Determine active template
        if (!State.currentTemplateId) {
            const active = State.templates.find(t => t.es_activa == 1);
            if (active) {
                State.currentTemplateId = active.id;
            } else if (State.templates.length > 0) {
                State.currentTemplateId = State.templates[0].id;
            }
        }

        // Step 2: Get config for the active template (only if there is one)
        if (State.currentTemplateId) {
            const configUrl = `${State.basePath}api/categorias/plantillas.php?action=get_config&categoria_id=${State.categoriaId}&plantilla_id=${State.currentTemplateId}`;
            ...
            if (configData.success && configData.config) {
                ...
                if (!cfg.posicion_destacado_x && cfg.destacado_posicion_x) {
        ...
        // Render Initial State (shows empty state if no templates)
        renderAll();

        // Configurar interact.js despues de renderizar marcadores
        initInteractJS();
    } catch (e) {
        console.error('Error en fetchConfig:', e);
        // Still render the UI even on error
        renderAll();
        initInteractJS();
    }
}
...
// ==========================================
// CATEGORÍA: Toggle "Usar plantilla del grupo"
// ==========================================
function initCategoryToggle() {
    const checkbox = document.getElementById('usarPlantillaGrupo');
    const configContent = document.getElementById('configContent');
    const grupoInfo = document.getElementById('grupoPlantillaInfo');
    const toggleContainer = document.getElementById('togglePlantillaGrupo');

    if (!checkbox) return;

    // Fetch initial state from API
    fetch(`${State.basePath}api/categorias/plantillas.php?action=list&categoria_id=${State.categoriaId}`)
        .then(r => r.json())
        .then(data => {
            if (data.success) {
                // If no own templates and no active own templates → using group
                const hasOwn = data.plantillas && data.plantillas.length > 0;
                const isUsingGroup = !hasOwn || !data.plantillas.some(p => p.es_activa == 1);
                
                // For now default to own templates if they exist
                checkbox.checked = false; // Not checked = using own templates
                updateToggleUI(false);
            }
        })
        .catch(() => {
            // Default: show own config
            updateToggleUI(false);
        });

    checkbox.addEventListener('change', async () => {
        const useGroup = checkbox.checked;
        
        try {
            const action = useGroup ? 'use_grupo' : 'use_propias';
            const res = await fetch(`${State.basePath}api/categorias/plantillas.php?action=${action}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: `categoria_id=${State.categoriaId}`
            });
            const data = await res.json();
            
            if (data.success) {
                updateToggleUI(useGroup);
                if (!useGroup) {
                    await fetchConfig();
                }
                ...
                // Revert
                checkbox.checked = !useGroup;
                notifyError(data.message || 'Error al cambiar modo');
            }
        } catch (e) {
            checkbox.checked = !useGroup;
    ...
    function updateToggleUI(useGroup) {
        if (configContent) configContent.style.display = useGroup ? 'none' : 'block';
        if (grupoInfo) grupoInfo.style.display = useGroup ? 'block' : 'none';
        if (toggleContainer) {
            toggleContainer.classList.toggle('active-grupo', useGroup);
        }
    }
}
...
function renderRibbon() {
    ...
    customTemplates.forEach(tpl => {
        ...
        thumb.innerHTML = `
            <img src="${State.basePath}uploads/categorias/${State.categoriaId}/${tpl.archivo}" style="width:100%; height:100%; object-fit:cover; border-radius:4px;">
...
function renderTemplateImage() {
    ...
    const path = `${State.basePath}uploads/categorias/${State.categoriaId}/${tpl.archivo}`;
...
async function saveConfig() {
    ...
    try {
        // Step 1: Upload pending firma file first (if any)
        if (State.pendingFiles.firma) {
            const firmaForm = new FormData();
            firmaForm.append('firma', State.pendingFiles.firma);
            firmaForm.append('plantilla_id', State.currentTemplateId || '');
            firmaForm.append('categoria_id', State.categoriaId);

            const firmaRes = await fetch(`${State.basePath}api/categorias/plantillas.php?action=upload_firma`, {
                method: 'POST',
                body: firmaForm
            });
            const firmaData = await firmaRes.json();
            if (firmaData.success && firmaData.firma_imagen) {
                State.config.firma_imagen = firmaData.firma_imagen;
            } else {
                console.warn('Error al subir firma:', firmaData.message);
            }
            State.pendingFiles.firma = null;
        }

        // Step 2: Save text config
        const formData = new FormData();
        formData.append('categoria_id', State.categoriaId);
        ...
        Object.keys(State.config).forEach(k => {
            ...
            } else if (!k.includes('imagen')) {
                formData.append(k, State.config[k]);
            }
        });

        const res = await fetch(`${State.basePath}api/categorias/plantillas.php?action=save_config`, {
            method: 'POST',
            body: formData
        });
...
async function uploadTemplate(e) {
    ...
    try {
        const res = await fetch(`${State.basePath}api/categorias/plantillas.php?action=upload`, {
            method: 'POST',
            body: formData
        });
        ...
        const data = await res.json();
        if (data.success) {
            State.currentTemplateId = data.plantilla?.id || data.plantilla_id; // API returns nested object
            await fetchConfig();
...
async function deleteActiveTemplate(id) {
    ...
    try {
        const res = await fetch(`${State.basePath}api/categorias/plantillas.php?action=delete`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `plantilla_id=${id}&categoria_id=${State.categoriaId}`
        });
        const data = await res.json();
...
function guardarSnapshotPreviewGrupo(dataUrl) {
    ...
    try {
        ...
        const payload = {
            categoria_id: Number(State.categoriaId) || null,
            plantilla_id: plantillaId,
            snapshot_data_url: dataUrl,
            generado_en: new Date().toISOString()
        };

        localStorage.setItem(`cce_snapshot_categoria_${State.categoriaId}_${plantillaId}`, JSON.stringify(payload));
        localStorage.setItem(`cce_snapshot_categoria_${State.categoriaId}_last`, JSON.stringify(payload));
    } catch (e) {
...
async function previewCertificate() {
    ...
    formData.append('id', State.categoriaId);
